...
import { GitHubBadge, CreatorBadge, FloatingGitHubButton } from "@/components/github-badge";
import type { DocumentInfo, PageGroup } from "@/types";
      ...
      >
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <motion.p 
            ...
            className="text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto"
          >
            Your PDF document has been loaded successfully
          </motion.p>
        </div>
          ...
          <div className="bg-card backdrop-blur-md border border-border rounded-xl p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{documentInfo?.pdf_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Total Pages:</span>
                <span className="font-medium">{documentInfo?.total_pages}</span>
              </div>
            </div>
            
            {/* Page Navigation */}
            <div className="mt-6">
              <h3 className="text-lg font-semibold mb-4">Select Page Range</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {pageGroups.map((group, index) => (
                  <Link
                    key={index}
                    href={`/page/1?start=${group.start}&end=${group.end}`}
                    className="bg-background hover:bg-primary/10 border border-border hover:border-primary/50 rounded-lg p-3 text-center transition-all"
                  >
                    <span className="text-sm">Pages {group.start}-{group.end}</span>
                  </Link>
                ))}
              </div>
              <Link
                href="/page/1"
                className="mt-4 inline-block w-full text-center bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg p-3 transition-colors"
              >
                View All Pages
              </Link>
            </div>
              ...
              <div className="bg-gradient-to-br from-primary/80 to-primary p-1 rounded-2xl shadow-xl group-hover:shadow-primary/20 transition-all duration-300 transform group-hover:scale-[1.02]">
                <div className="bg-card backdrop-blur-sm rounded-xl p-8 h-full flex flex-col justify-between border-t border-border/10">
                  <div>
                    ...
                    <h3 className="text-2xl font-bold mb-2 group-hover:text-primary transition-colors">Upload New PDF</h3>
                    <p className="text-muted-foreground mb-6">Replace current document with a new PDF file.</p>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-primary/80">Upload new</span>
                    <div className="w-10 h-10 rounded-full bg-primary/30 flex items-center justify-center group-hover:bg-primary transition-colors">
                      <svg className="w-5 h-5 text-primary-foreground transform group-hover:translate-x-1 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                      </svg>
              ...
              <div className="bg-gradient-to-br from-primary/60 to-primary/80 p-1 rounded-2xl shadow-xl group-hover:shadow-primary/20 transition-all duration-300 transform group-hover:scale-[1.02]">
                <div className="bg-card backdrop-blur-sm rounded-xl p-8 h-full flex flex-col justify-between border-t border-border/10">
                  <div>
                    ...
                    <h3 className="text-2xl font-bold mb-2 group-hover:text-primary transition-colors">Load Different Document</h3>
                    <p className="text-muted-foreground mb-6">Access another document using its access token.</p>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-primary/70">Switch document</span>
                    <div className="w-10 h-10 rounded-full bg-primary/30 flex items-center justify-center group-hover:bg-primary transition-colors">
                      <svg className="w-5 h-5 text-primary-foreground transform group-hover:translate-x-1 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                      </svg>
